...
def graph_to_boiling_point(button, ndim):
    ...
    """
    This function calculates the methyl index, Wiener path numbers and Hosoya index using the function
    graph_to_descriptors, retrieves the current optimal parameters from the relevant file and use all the aforementioned
    data to calculate the boiling point by calling the function boiling_point.
    """
...
def append_to_database(button, ndim):
    ...
    """
    This function creates the by default data base using the function build_default_dataset if it doesn't already exists,
...
def learn_from_new_set():
    ...
    the function cost_function using the scipy minimize routine with a Nelder-Mead algorithm,  and if the new set of
    parameters indeed yield a lower standard deviation, the parameter file is replaced with a file containing the new
    parameters.
    """
...
def graph_to_descriptors(button, ndim):
    ...
    the function build_Cindex, (2) calculates the distance matrix using the function distance_matrix_builder, (3)
    calculates the methyl index using the function methyl_index_calculator. Finally, calculates the path numbers stored
    in a vector (pathvector[i] corresponds to the path number for atoms separated by i bonds) using the function
    path_number_calculator.
    """
    ...
    methylindex = methyl_index_calculator(distancematrix, ncarb)
...
def methyl_index_calculator(distancematrix, ncarb):
    """
